...
-- Q1. List Top 3 products based on QuantityAvailable
select productid,productname,quantityavailable
from products
order by quantityavailable
limit 3;
-- Q2. Display EmailId of customers who have done more than ten purchases
select
emailid,
...
-- Q3. List the Total QuantityAvailable category wise in descending order.
SELECT 
    c.CategoryName,
    SUM(p.QuantityAvailable)  AS Total_Quantity
FROM Products p
JOIN Categories c 
    ON p.CategoryId = c.CategoryId
GROUP BY c.CategoryName
ORDER BY Total_Quantity DESC;
 -- Q4. Display ProductId, ProductName, CategoryName, Total_Purchased_Quantity
-- for the product which has been sold maximum in terms of quantity.
SELECT 
    p.productid,p.Productname,c.categoryname,
    sum(pd.quantitypurchased) as Total_Quantity
...
limit 1;
-- Q5. Display the number of male and female customers in FastKart.
select 
gender,
count(*)
as Total_customers
from users
group by gender;
-- Q6. Display ProductId, ProductName, Price and Item_Classes of all the products
-- where Item_Classes are defined as:
-- If the price of an item is less than 2000 then "Affordable"
-- If the price of an item is between 2000 and 50000 then "High End Stuff"
-- If the price of an item is more than 50000 then "Luxury"
select
...
End As Item_class
from products;
-- Q7. Write a query to display ProductId, ProductName, CategoryName,
-- Old_Price and New_Price as per the following conditions:
-- If the category is "Motors", decrease the price by 3000
-- If the category is "Electronics", increase the price by 50
-- If the category is "Fashion", increase the price by 150
select
p.productid,p.productname,c.categoryname,p.price AS Old_price,
case
when c.categoryname = "Motors" then p.price - 3000
when c.categoryname = "electronis" then P.price + 50
...
join categories c
on p.categoryid = c.categoryid;
-- Q8. Display the percentage of females present among all users.
-- Round the percentage to 2 decimal places and append '%' sign.
SELECT 
    CONCAT(
        ROUND(
            (COUNT(CASE WHEN Gender = 'F' THEN 1 END) * 100.0) / COUNT(*),
            2
        ),
        '%'
    ) AS Female_Percentage
FROM Users;
-- Q9. Display the average balance for both card types
-- where CVVNumber > 333 and NameOnCard ends with the alphabet 'e'.
select
    CardType,
...
GROUP BY CardType;
-- Q10. What is the 2nd most valuable item available which does not belong
-- to the "Motor" category.
-- Value of an item = Price * QuantityAvailable
-- Output columns: ProductName, CategoryName, Value
SELECT
    p.ProductName,
    c.CategoryName,
...
select
p.productname,c.categoryname,(p.price*p.quantityavailable) as Value
from products p
join categories c
on p.categoryid = c.categoryid
where c.categoryname <> "motors"
